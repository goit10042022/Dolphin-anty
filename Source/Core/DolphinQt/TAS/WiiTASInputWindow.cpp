...
WiiTASInputWindow::WiiTASInputWindow(QWidget* parent, int num) : TASInputWindow(parent), m_num(num)
{
  ...
  // A lifted board shows about -6kg.
  constexpr int min_total_kg = -8;
  ...
  auto* const balance_value_tl = CreateWeightSliderValuePair(
      ...
      max_sensor_value, balance_tl_keyseq, m_balance_board_box);
  auto* const balance_value_tr = CreateWeightSliderValuePair(
      ...
      max_sensor_value, balance_tr_keyseq, m_balance_board_box);
  ...
  auto* const balance_value_bl = CreateWeightSliderValuePair(
      ...
      min_sensor_value, max_sensor_value, balance_bl_keyseq, m_balance_board_box);
  auto* const balance_value_br = CreateWeightSliderValuePair(
      ...
      min_sensor_value, max_sensor_value, balance_br_keyseq, m_balance_board_box);

  auto* bal_weight_layout = new QHBoxLayout;
  auto* const total_weight_value = CreateWeightSliderValuePair(
      bal_weight_layout, min_total_kg, max_total_kg, balance_weight_keyseq, m_balance_board_box);
...
void WiiTASInputWindow::LoadExtensionAndMotionPlus()
{
  ...
  if (Core::IsRunning(Core::System::GetInstance()))
  {
    ...
    if (m_num == WIIMOTE_BALANCE_BOARD)
    {
    ...
    else
    {
      m_is_motion_plus_attached =
          static_cast<WiimoteEmu::Wiimote*>(wiimote)->GetMotionPlusSetting().GetValue();
    }
  }
  ...
  if (m_num != WIIMOTE_BALANCE_BOARD)
  {
    auto* const wm = static_cast<WiimoteEmu::Wiimote*>(wiimote);

    m_motion_plus_callback_id = wm->GetMotionPlusSetting().AddCallback([this](const bool attached) {
      QueueOnObject(this, [this, attached] { UpdateMotionPlus(attached); });
    });
    m_attachment_callback_id =
        wm->GetAttachmentSetting().AddCallback([this](const int extension_index) {
...
void WiiTASInputWindow::hideEvent(QHideEvent* const event)
{
  ...
  {
    auto* const wm = static_cast<WiimoteEmu::Wiimote*>(wiimote);
    wm->GetMotionPlusSetting().RemoveCallback(m_motion_plus_callback_id);
    wm->GetAttachmentSetting().RemoveCallback(m_attachment_callback_id);
  }
