...
void CustomAssetLoader2::WorkerThreadRun()
{
  ...
  while (!m_exit_flag.IsSet())
  {
    ...
    while (!m_pending_assets.empty() && !m_exit_flag.IsSet())
    {
      ...
      if ((m_current_asset_memory + last_completed_memory) > m_max_memory_allowed)
        break;
      ...
      if (item->Load())
      {
        ...
        last_completed_memory = m_completed_asset_memory;
...
std::vector<std::size_t> CustomAssetLoader2::LoadAssets(std::list<CustomAsset*> pending_assets,
                                                        u64* memory, u64 max_memory_allowed)
{
  std::vector<std::size_t> completed_asset_session_ids;
  {
    std::lock_guard<std::mutex> guard(m_completed_work_lock);
    *memory += m_completed_asset_memory;
  ...
  {
    std::lock_guard<std::mutex> guard(m_pending_work_lock);
    std::swap(m_pending_assets, pending_assets);
    m_current_asset_memory = *memory;
