...
bool EditorMain::RebuildState()
{
  ...
  if (!AddTemplate(fmt::format("{}/raster_material.json", templates_path_root), "raster_material",
                   m_state.get()))
  {
    return false;
  }
  if (!AddTemplate(fmt::format("{}/raster_shader.json", templates_path_root), "raster_shader_meta",
                   m_state.get()))
  {
    return false;
  }
  if (!AddTemplate(fmt::format("{}/raster_shader.ps.glsl", templates_path_root), "raster_shader_ps",
                   m_state.get()))
  {
    return false;
  }
  if (!AddTemplate(fmt::format("{}/raster_shader.vs.glsl", templates_path_root), "raster_shader_vs",
                   m_state.get()))
  {
    return false;
  }
  ...
  asset_loader.Reset();
  custom_resource_manager.Reset();

  m_asset_reload_event = EditorEvents::AssetReloadEvent::Register(
      [&custom_resource_manager, this](const VideoCommon::CustomAssetLibrary::AssetID& asset_id) {
        if (asset_id != "")
        {
          custom_resource_manager.ReloadAsset(asset_id);
          OnChangeOccured();
        }
      },
      "EditorMain");
...

  {
    ...
    highlight_shader_asset_map.try_emplace("metadata", pipeline_path_root_fs / "highlight" /
                                                           "color.rastershader");
    highlight_shader_asset_map.try_emplace("pixel_shader",
                                           pipeline_path_root_fs / "highlight" / "color.ps.glsl");
    highlight_shader_asset_map.try_emplace("vertex_shader",
                                           pipeline_path_root_fs / "highlight" / "color.vs.glsl");
    ...
    highlight_material_asset_map.try_emplace("", pipeline_path_root_fs / "highlight" /
                                                     "highlight.rastermaterial");
    m_state->m_editor_data.m_asset_library->SetAssetIDMapData(
        "editor-highlight-material", std::move(highlight_material_asset_map));
  }

  {
    ...
    simple_light_visual_shader_asset_map.try_emplace("metadata",
                                                     pipeline_path_root_fs / "light_visualization" /
                                                         "simple-light-visualization.rastershader");
    simple_light_visual_shader_asset_map.try_emplace("pixel_shader",
                                                     pipeline_path_root_fs / "light_visualization" /
                                                         "simple-light-visualization.ps.glsl");
    simple_light_visual_shader_asset_map.try_emplace("vertex_shader",
                                                     pipeline_path_root_fs / "light_visualization" /
                                                         "simple-light-visualization.vs.glsl");
    m_state->m_editor_data.m_asset_library->SetAssetIDMapData(
    ...
    simple_light_visual_material_asset_map.try_emplace(
        "", pipeline_path_root_fs / "light_visualization" /
                "simple-light-visualization.rastermaterial");
    m_state->m_editor_data.m_asset_library->SetAssetIDMapData(
        "editor-lighting-vis-material", std::move(simple_light_visual_material_asset_map));
  }
