                                    ...
                                    const std::filesystem::path& new_path)
{
  ...
  else if (ext == ".rastershader")
  {
    ...
    const auto old_root = old_path.parent_path();
    const auto old_name = old_path.stem();
    const auto old_pixel_shader_path = (old_root / old_name).replace_extension(".ps");
    if (std::filesystem::exists(old_pixel_shader_path))
    {
      const auto new_root = new_path.parent_path();
      const auto new_name = new_path.stem();
      const auto new_pixel_shader_path = (new_root / new_name).replace_extension(".ps");
    ...
    const auto old_vertex_shader_path = (old_root / old_name).replace_extension(".vs");
    if (std::filesystem::exists(old_vertex_shader_path))
    {
      const auto new_root = new_path.parent_path();
      const auto new_name = new_path.stem();
      const auto new_vertex_shader_path = (new_root / new_name).replace_extension(".vs");
