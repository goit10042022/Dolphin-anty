                                                 ...
                                                 const CustomPipelineMaterial& custom_material)
{
  ...
  public:
    ...
    bool Compile() override
    {
      ...
      for (const auto& texture_resource : m_custom_material.shader.material->textures)
      {
        const auto& sampler =
            m_custom_material.shader.shader_data->m_pixel_samplers[texture_resource.sampler_index];
        ...
        shader_code.Write("SAMPLER_BINDING({}) uniform {} samp_{};\n",
                          custom_sampler_index_offset + texture_resource.sampler_index,
